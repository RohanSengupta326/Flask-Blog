...
@app.route('/')
def home():
    posts = Myblog.query.filter_by().all()
...
def delete(sno):
    if "user" in session and session['user'] == params['uname']:
        post = Myblog.query.filter_by(sno=sno).first()
...
def edit(sno):
    if "user" in session and session['user'] == params['uname']:
        if request.method == "POST":
            ...
            if sno == '0':
                post = Myblog(title=title, slug=slug,
                              content=content, img_name=img_file, date=date)
                db.session.add(post)
                db.session.commit()
                return redirect('/login')
            else:
...
def post_route(post_slug):
    post = Myblog.query.filter_by(slug=post_slug).first()
...
def contact():
    if(request.method == 'POST'):
        ...
        db.session.commit()
        mail.send_message('Hey! New message from "Code Like a Pro"',
                          sender=params['gmail_user'],
                          recipients=[email],
                          body=params['mail_msg']
                          )
